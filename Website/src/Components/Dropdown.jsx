...
function Dropdown({ title, items }) {
    ...
    const handleMouseLeave = () => {
      const id = setTimeout(() => setIsOpen(false), 150); // Delay to allow smooth movement
      setTimeoutId(id);
    };
    ...
    return (
      ...
      >
        <div className="cursor-pointer text-gray-800 text-xs px-5 py-2 rounded-md bg-white shadow-md hover:bg-gray-200 hover:text-orange-500 flex items-center justify-center transition-all duration-300 h-[32px] min-w-fit">
          <span>{title}</span>
          <svg
...
function DropdownItem({ item, handleClick, isLast }) {
    ...
    return (
      <div className="relative" onMouseEnter={onEnter} onMouseLeave={onLeave}>
        <div
          className={`px-4 py-3 text-sm font-medium flex justify-between items-center text-gray-800 bg-white border-b border-gray-200 hover:bg-orange-50 hover:text-orange-600 transition-colors duration-200 ${
            isLast ? "border-b-0" : ""
          }`}
        ...
        {hasSubItems && isSubOpen && (
          <div className="absolute top-0 left-full ml-1 bg-white shadow-xl rounded-lg min-w-[250px] z-[9999] py-2 border border-gray-300">
            {item.subItems.map((subItem, idx) => (
              <div
                key={idx}
                className={`px-4 py-3 text-sm font-medium text-gray-800 bg-white border-b border-gray-200 hover:bg-orange-50 hover:text-orange-600 cursor-pointer ${
...
function DropdownMenu() {
  return (
    <nav className="flex gap-2 relative z-[1000]">
